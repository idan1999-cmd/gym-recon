...
2. **Single Travel Allowance Principle (תשלום נסיעות יחיד בלבד):**
   - עובד בעל תפקיד כפול (כמו לאון) מקבל תשלום נסיעות **פעם אחת בלבד** (תחת קבלה עמודה D, ומאופס `None` תחת מדריך עמודה K).
   - מדרגות נסיעות: `<60` שעות = 100 ₪, `60–90` שעות = 150 ₪, `>90` שעות = 200 ₪, לאוניד ורחובסקי = 208.50 ₪ לפי חוזה.
