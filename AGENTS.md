...
12. **Bonus Exclusion from Shift Hours (הפרדת בונוסים משעות עבודה):**
   - בחישוב שעות מאמנים וקבלה (שורות 25 ו-26 בגיליון `דוח מרכז לאישור מנהל`), נוסחת השכר מחשבת **אך ורק שעות עבודה בפועל (רגילות/נוספות) ותשלום נסיעות (Row 11)**, ולעולם **לא** כוללת את שורת הבונוסים (Row 13 / `+E13`).
   - כל רכיבי הבונוס והמאמץ המיוחד מרוכזים בנפרד בתחתית הגיליון תחת **סעיף תקציבי `22662 עמלות מכירות מנויים ושידרוגים`** (שורה 62).
